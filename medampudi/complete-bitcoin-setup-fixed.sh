...
create_full_docker_compose() {
    ...
    # Create full version WITHOUT version key (deprecated)
...
start_all_services() {
    ...
    # First, let's check if we need to handle the existing network
    if docker network ls | grep -q bitcoin_bitcoin; then
        log_info "Existing Docker network found, handling gracefully..."
        
        # Stop Bitcoin container temporarily to free the network
        log_info "Temporarily stopping Bitcoin container..."
        docker compose stop bitcoind || true
        
        # Remove the old network
        docker network rm bitcoin_bitcoin 2>/dev/null || true
    fi
    
    # Start Bitcoin first with the new compose file
    log_info "Starting Bitcoin Core with updated configuration..."
